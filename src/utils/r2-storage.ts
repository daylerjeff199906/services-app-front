 ...
 */
export async function uploadToR2(file: File, serviceId: string): Promise<string> {
  if (!R2_BUCKET_NAME) {
    throw new Error("R2 bucket name is not configured.")
  }

  // Generate a safe unique key: services/{serviceId}/{timestamp}-{filename}
  const cleanFileName = file.name.replace(/[^a-zA-Z0-9.-]/g, "_")
  const key = `services/${serviceId}/${Date.now()}-${cleanFileName}`
