  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newUser = {
        ...
        email: email,
        name: name,
        role: "SERVICE_OWNER" as const,
      }

      const defaultServices = [
        { id: "srv_3", name: `Servicio de ${name}`, slug: "mi-servicio-inicial", description: "Tu primer espacio de gestión" },
      ]

      login(newUser, defaultServices)
    ...
    <form onSubmit={handleSubmit} className={cn("flex flex-col gap-6", className)} {...props}>
      <FieldGroup>
        ...
        <Field>
          ...
          />
          {errors.password ? (
            <p className="text-xs text-destructive mt-1 font-semibold">{errors.password}</p>
          ) : (
